...
// Disposing the datasource must cancel lookups that have already detached from
// their caller, so they cannot keep using a closed client and a stale API key.
func TestNominalCatalogCloseCancelsDetachedLookup(t *testing.T) {
	const assetRid = "ri.scout.main.asset.disposed"
	...
	// Release the handler before Close waits on it.
	defer server.Close()
	defer close(release)
	...
	go func() {
		_, err := catalog.FetchAssetByRid(context.Background(), config, assetRid)
		result <- err
	}()

	waitForTestSignal(t, arrived, "backend request to arrive")
	catalog.close()

	select {
	case err := <-result:
		if err == nil {
			t.Fatal("FetchAssetByRid returned nil error, want the lookup to fail after close")
		}
	case <-time.After(catalogTestTimeout):
		t.Fatal("timed out waiting for close to cancel the detached lookup")
	}
}

...
// flightWaitContext reports the moment its caller starts waiting on a flight.
// This assumes ttlCache.get reads ctx.Done() only after DoChan has registered
// the caller, so the first Done() call marks that moment. If get ever reads
// Done() earlier, the probe fires early and the surviving-caller tests report
